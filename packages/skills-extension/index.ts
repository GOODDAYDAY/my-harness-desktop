// skills-extension —— pi 内核的技能播报扩展。
//
// 跑在 pi 进程里,是"内核负责读"的 pi 侧实现:session_start 时扫 pi 自己的存储
// (settings.json + skills 目录),算完整列表(含禁用),写播报文件 ~/.pi/agent/desktop-skills.json。
// 桌面侧 pi-skill-provider 读这个播报文件,壳不碰 pi 的任何存储。
// 交付:client/pi/skills-extension-installer.ts 同步本目录到 ~/.pi/agent/extensions/skills-extension/。
...
export default function (pi: ExtensionApi): void {
  ...
  pi.on("session_start", () => {
    ...
    try {
      ...
      console.error("[skills-extension] scan failed:", err);
    }
  });
}
