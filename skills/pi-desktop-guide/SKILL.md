---
name: pi-desktop-guide
description: pi-desktop 桌面端的体系速查与排障指南。当用户在 pi-desktop 里询问这个应用本身的功能、配置位置、插件体系、skills 管理,或遇到桌面端行为疑问时使用。
---

# pi-desktop 速查

pi-desktop 是 pi 底座的 Electron 桌面壳:薄内核(加载器/槽位/权限/RPC) + 插件内容层。
本 skill 只覆盖"这个桌面应用本身"的事实,不覆盖 pi 底座的通用用法。

## 配置与数据位置

- 桌面配置区:`~/.pi-desktop/`(config/ 偏好与插件配置、plugins/ 用户级插件、skills/ 内置 skills 受管目录)
- 底座配置:`~/.pi/agent/settings.json`(models、skills[] 等底座字段)
- 桌面偏好(主题/字号/语言):electron-store,存于 `~/.pi-desktop/config/`
...
- 内置 skills:`~/.pi-desktop/skills/`,随 app 升级强制覆盖(受管目录,要改请 fork 到自己的 skills 目录);总开关挂/摘 settings.json 里的源路径条目。
