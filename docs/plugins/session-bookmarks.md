...
**失败回滚**：步骤 1 失败（校验）什么都不产生；步骤 2 内部任何失败（复制失败、pi 启动失败、fork RPC 抛错）由框架回滚——恢复先前上下文、停掉跑在中间副本上的 pi、删中间副本，任何路径不留孤儿。恢复先前上下文以"激活态还停在中间副本"为前提：fork 的 start/send 等待窗口内用户切走上下文（点别的会话/⌘N/切目录）时，框架按并发中止处理——fork 命令钉在中间副本自己的进程上（别的会话物理上收不到），不劫持用户新选择、不恢复先前上下文，仅停进程删副本；若 fork 已执行，产物留在会话列表里可自行打开。面板顶部错误条显示错误消息 + 重试/关闭按钮，重试对同一个收藏重新执行完整流程，上一次残留已被回滚清掉。
...
框架自动回滚。`forkFromSession` 的 catch 里停掉跑在中间副本上的 pi、删中间副本，并在激活态仍停在中间副本时恢复先前上下文（被并发切走则不拽回用户选择）——任何失败路径都不留孤儿文件、不留泄漏进程。插件侧的前置校验（锚点必须是 user 消息）在更早一步失败，什么都不产生。面板顶部错误条显示错误消息和重试/关闭按钮，重试是干净的（上一次残留已被回滚清掉）。
