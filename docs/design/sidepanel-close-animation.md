...
> 状态：已实现
> 关联：`src/api/renderer/components/right-panel.tsx`（`RightPanelContent`）、`src/api/renderer/index.css`（`.sidepanel-panel-exit`）、react-resizable-panels `^2.1.9`
...
所以移除 exiting panel 的瞬间，必须给活跃 panel 传
`defaultSize={移除瞬间记录的当前尺寸}`——库自身分配逻辑会先分配 defaultSize
做精确恢复，剩余 0 即无重分。这是本设计的最后一块：缺了它，R 大小不管
收起到多幺流畅，绝望之跳永远不会好了。

**实证补充（移除瞬间的尺寸恢复）**：库移除 panel 时默认行为是"剩余均分"
（autoSaveId 的 panelKey 由 id 集合决定；id 集合变化 → localStorage 恢复
失败 → `calculateUnsafeDefaultLayout` 全量重分配，剩余 panel 尺寸会跳变）。
所以移除 exiting panel 的瞬间，必须给活跃 panel 传
`defaultSize={移除瞬间记录的当前尺寸}`——库自身分配逻辑会先分配 defaultSize
做精确恢复，剩余 0 均分即无变化。这是本设计的最后一块：没有它，ency 收起到
0 的瞬间重分配仍然跳班。
...
**决策四：动画时序用 rAF 驱动，不用 `panel.collapse()`（库内部动画），
不用 CSS keyframes。**

`panel.collapse()` 动画完成没有回调——库内部动画时长是实现细节，不可知。
要靠它只能用 setTimeout 赌时长。现有实现（G-20260201-03）就是这么赌的，
而且赌错了：赌 240ms 但 exit keyframes 只有 `--motion-duration-fast: 120ms`，
幽灵 panel 多挂 120ms 才卸载。

rAF 链由自己控制：**结束帧是确定性完成信号**，不赌库内部动画时长、不赌
CSS keyframes 时长（§3.6 事件驱动纪律）。每帧
`panelRef.resize(startSize × (1 − easeInOutCubic(t)))`，240ms。
...
- closing 期间再点同一 strip 图标（重新激活）：该 id 从 closingIds 移除，
  rAF 链在下一帧因 state 已移除而自然终止…… **演进**：终止路径靠 effect
  清理函数 cancelAnimationFrame。本期实现带 cleanup。
