...
页面用一个紧凑的「来源路径」筛选器（可折叠、多选：checkbox 列表，一目录一项 = 路径 + 技能计数），默认全选，取消勾选即排除该路径；下方列表只显示仍勾选的路径。来源路径**只做透传展示 + 相等过滤**，不反推、不写死——壳只把内核扫描回报的 `sourceDir`/`filePath` 当成字符串，不把路径变成壳自己的分类。内置 Skills 目录不再单独成卡：它就是普通一条来源路径出现在筛选列表里（技能行的 `source` 徽标仍标 `builtin`）。

技能列表是**单一扁平列表**（主内容），被「路径多选 + 启用/禁用筛选 + 搜索」三路叠加过滤。每行挂 `scope` 徽标（全局/当前项目）和 `source` 徽标，SKILL.md 绝对路径（`filePath`）作为副行原样展示。排序：全局在前、项目在后，同作用域内按 `sourceDir`、再按名字。

筛选与搜索：路径多选、启用/禁用筛选（全部/启用/禁用）、文本搜索三者叠加；文本搜索匹配 `name`、`description`、`sourceDir`、`filePath`。筛选器头显示当前选择（全部路径 / 已选 N 个路径），部分选中时提供「全选」。

### 6.2 开关按能力标志渲染

每个技能行的开关数量随内核能力走，不随内核身份走。pi 和 dsh 都是两枚（启用/禁用、固定到上下文），能力标志 `toggleEnabled`/`toggleModelInvocable` 全 true。禁用项整行置灰、开关仍可点（点了就是重新启用）。开关动作落 `setEnabled`/`setModelInvocable`，壳不管背后写哪，只等内核回报变化后刷新。
