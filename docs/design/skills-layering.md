...
页面先把**所有扫描到的来源路径**做成一个纯过滤列表（一目录一行：路径 + 技能计数，点选即过滤），再在下方列出技能列表。点「全部路径」回到全量。这个路径列表**只做透传展示 + 相等过滤**，不混入任何开关、徽标——开关的职责和过滤的职责分开，避免"有的路径能开关、有的不能"的困惑。内置 Skills 的启停是一个**独立控件**（见下），不属于路径过滤。

内置 Skills 目录单独成一张小卡片：标题「内置 Skills」+ 路径 + 一句说明（"随壳分发的内置技能目录，关闭后不再扫描"）+ 启停开关（`setBundledEnabled`）。它是"是否把这个目录扫进来"的设置，不是"看哪个目录"的过滤，所以不放进路径列表。启用后该目录会作为一条 `sourceDir` 出现在路径列表里（`source: "builtin"`）。

技能列表是**单一扁平列表**，不按 `scope`/`sourceDir` 拆分组 section。每行挂 `scope` 徽标（全局/当前项目）和 `source` 徽标，SKILL.md 绝对路径（`filePath`）作为副行原样展示。排序：全局在前、项目在后，同作用域内按 `sourceDir`、再按名字。

筛选与搜索：启用/禁用筛选（全部/启用/禁用）与文本搜索叠加生效；文本搜索匹配 `name`、`description`、`sourceDir`、`filePath`。来源路径是内核扫描回报的 `sourceDir`/`filePath`，壳不反推路径——不把路径变成壳自己的分类。
