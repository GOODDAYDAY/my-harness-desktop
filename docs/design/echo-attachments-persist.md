...
持久化的徽章只留展示所需的最小字段：`{ seq, quotePreview, comment }`——`messageId`（引文锚点）不落盘，因为恢复出来的徽章没有交互场景（篮子已清，编辑/跳原文都是发送前的动作；§4.5）。截断规则：`quotePreview ≤ 60`、`comment ≤ 160`——徽章本就是预览，生产侧本来就有截断（`truncate(quote, 60)`）。
...
持久化只留 `{seq, quotePreview, comment}`，不落 `messageId`（引文锚点）——恢复出的徽章是纯展示。跳原文、编辑评论都是发送前的动作，发送后篮子清空、交互路径已经不存在；为恢复态加锚点跳转是扩范围，不做。
