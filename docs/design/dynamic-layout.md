...
> 读者前提：本文是 my-harness-desktop 的单特性设计文档，面向本项目贡献者。槽位契约、PluginContext、事件总线、pluginsNonce 等体系概念见 docs/DESIGN.md；文中引用的文件路径均以仓库根为基准（`src/`、`packages/` 下）。首次出现的体系概念随文给一句话锚点，不展开复述。
...
文件展示流和会话流并行独立——这是 VSCode/Cursor 这一代工具的主流模型：编辑器区是多 tab、可分屏的视图容器，会话只是其中一个视图，不是整个中区。落到 my-harness-desktop：
