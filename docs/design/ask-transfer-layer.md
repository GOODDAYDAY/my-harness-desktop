...
> **2026-08-19 首版**：把 `ask_user_question` 的"提问往返"（内核挂起 → 用户作答 → 答案回灌）收敛到一层 TransferLayer，让内核专属形状不穿透到桌面。
...
`BaseBackend.answerQuestion`（`backend.ts`）签名改为中性答案数组：
...
| **pi 适配器** | `client/pi/pi-backend.ts` + `rpc-adapter.ts` | **翻译归位**：`extension_ui_request`（method=select/input 时）→ 中性 `QuestionRequestEvent`；override `answerQuestion` = `extension_ui_response` 帧翻译 |
| **dsh 适配器** | `client/dsh/dsh-backend.ts` + `dsh-question-bridge.ts` | **翻译归位**：override `answerQuestion` = 写答案文件（阶段一）；提问事件源在 `client/dsh` 内 fs.watch 问句目录 → 投中性事件 |
| 应用 | `core/application/sessions/session-store.ts` | 删掉 dispatch pi 专属 `extensionUI` 的分支，统一订阅 backend 的中性提问事件；暴露统一 `answerQuestion` 分发 + `injectQuestion` 接收外部提问 |
| 流入 | `api/ipc/dsh-questions.ts` | **删除**（翻译逻辑收进 `client/dsh`） |
...
### 6.1 阶段一：翻译归位 + 渲染纯函数化（不改 dsh 内核，本期已交付）
...
### 6.2 阶段二：dsh 内核补面（可选，改 deepseek-harness，见 dsh-sdk-server-supplement.md）

补 SDK server `session/answer`（+ `session/listTools`），dsh 走 DSH 原生 `ctx.userQuestions.ask` 机制，废文件侧车桥。

## 7. 验收标准

1. **纪律 2（专属形状不穿透）**：`core/application`、`api/`、插件 renderer 下零 `extension_ui_request`/`.my-harness-desktop-questions` 字面量；翻译只出现在 `client/pi`、`client/dsh`。
...
| 2 | dsh 侧车桥用 fs.watch 还是轮询 | fs.watch（事件驱动优先） | §3.6 |
| 3 | 阶段二（改 dsh SDK server）是否本期做 | 不纳入，独立排期（见 dsh-sdk-server-supplement.md） | 需改 deepseek-harness（另一仓库） |
