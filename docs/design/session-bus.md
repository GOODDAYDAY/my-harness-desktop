...
- **`stream`**——全量事件流 = lifecycle 五个边界事件 + 全部增量（`messageStart`、`messageUpdate`、`toolCallStart/Update/End`、`turnStart/End` 等）。**正当消费者是 plugin 地址**（监控面板：消费者是人，没有上下文成本）；deliverTo 是 session 时路由器把 stream 降级为 lifecycle，并回给订阅方一条 `kind: "tap_degraded"` 的系统事件帧说明降级原因——agent 上下文是稀缺资源，stream 灌进 agent 等于让接收方为发送方的执行细节付 token 账，这是 §4 论证要防的事，路由器替粗心的编排方兜底。
  ...
  **落地消费者（2026-08-08）**：sub-agent 插件的对话面板（`subagent-scheduling.md` §6.5）——用户经面板 tap 目标会话的 stream 看流式进度/回复。这正是本节论证的"plugin 消费者是人、无上下文成本"的首次落地：人看流式进度被 §4 认可（"中间过程想不想看是人的需求"），面板把它接到 UI 上，且不打断用户当前会话流（面板是独立窗口，不 setContext 切走）。
...
完整输出默认全量交付，只有一条例外：**给 agent 接收方的帧**输出过长时路由器截断——按字符数估算 token（4 字符≈1 token），超过 8000 token 时保留头部 1/4（任务上下文）与尾部 3/4（结论密集区），中段以省略标记折叠，并附上会话文件的绝对路径——接收方真要全文，自己用 `read` 工具按路径取。截断保语义（头尾在，结论在），路径保完整（全文可达），两者缺一才会真的丢信息。

**2026-08-08 修订（交付分层）**：旧文把"给 agent 的帧"和"用户的交付物"混用同一个截断策略——实证反馈（"截断是垃圾操作"）指向后者：用户要的是完整结果，不是头尾保语义。分层后：

- **agent 接收方**（编排方 agent / 父会话）：截断保留——上下文是稀缺资源，N 个子的全量输出灌进 agent 是 token 账，截断 + 路径自读是正确语义。
- **用户交付物**（对话面板、卡片展开、会话文件）：**全量、零截断**——采集本就是全量（子会话文件有全文），交付给用户的帧不再切。`session_done` 帧的 output 全量 + sessionPath 恒带（接收方要全文可直接读文件）。

截断的位置从"交付帧"收窄到"进 agent 上下文的帧"，用户侧永不丢内容。
