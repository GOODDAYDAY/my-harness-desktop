...
四者互补，互不替代。内容图钉不做 fork（那是书签），不带文字（那是 review），不跨会话列内容（那是会话图钉的层）。
...
```json
{
  "/Users/.../sessions/abc-123.jsonl": [
    { "id": "7f3e...", "messageId": "msg-entry-id", "color": "#f38ba8", "x": 92, "y": 8 },
    { "id": "a1b2...", "messageId": "msg-entry-id-2", "color": "#89b4fa", "x": 40, "y": 55 }
  ]
}
```
...
按 sessionPath 分桶带来天然过滤：只有当前打开会话的钉参与渲染和面板展示。切会话不需要清数据，messages 数据源换了，扫描目标自然换。

### 3.3 同色替换与孤儿处置

同色替换规则逐字平移：**同一条消息上同一颜色只保留一个钉**，后钉的顶掉先钉的（旧钉弹飞、新钉落下，350ms 动画重叠期）。不同消息、不同颜色随意共存。

孤儿钉：messageId 在当前 messages 里不存在——rewind 截断了后续消息、连续重试失败被 `collapseRetryFailures` 折进 divider、手动编辑过 JSONL——扫描时 `querySelector` 找不到元素，钉不渲染。数据留在 config 里不主动清：折叠类消失是临时的（开"显示隐藏消息"即回来），截断类消失是永久的但不产生噪音（面板同样不列）。这与会话图钉对"行不在 DOM 就不显示"的处理一致。
...
- **会话钉段**：现有列表，原样。
- **内容钉段**：当前会话的内容钉，按消息在会话流中的先后排序（按 messageId 在 messages 里的 index，不按钉入时间——回看的认知顺序是内容顺序）。每条显示色点、消息预览（消息文本块原文前 30 字，从 `useSessionStore` 的 messages 取，与 messageActions 的 rowText 同取法）、定位按钮。列出口径与渲染口径一致：messageId 在当前 messages 里且该消息实际渲染（未被折叠/过滤）的钉才进列表——孤儿钉（§3.3）不进，"无内容钉"也按这个口径判定：config 里有钉但全是孤儿时，本段视同为空。

当前会话无内容钉时该段整体不渲染，不留空标题。段内过滤沿用色板 filter tab——现有"按颜色过滤"对两段同时生效。

### 6.2 点击定位

点内容钉条目调 `ctx.events.invoke("timeline:scrollTo", { messageId })`。这个通道是现成的：timeline 侧按 messageId 找 index、`scrollToIndex` 平滑滚动，找不到时登记 `pendingScrollRef` 等下轮 messages 变化兜底——session-tree 的 locate 按钮走的就是这条路。

两种降级情况：

- 消息被 `collapseRetryFailures` 折进 divider，或被显示开关过滤：找不到 index，兜底也等不到，静默不跳。这是 timeline scrollTo 的既有语义，内容钉不新增处理——钉在这种消息上本就随折叠隐藏（§3.3），面板也不列，用户点不到。

- 钉入后从未渲染过的消息（长会话顶部）：Virtuoso 的 `scrollToIndex` 按数据 index 跳，不依赖 DOM 已渲染，正常到达。

### 6.3 不跨会话

内容钉面板只列当前会话。跨会话的内容标记不需要——"哪个会话值得回"由会话图钉承担，内容钉是会话内的层。跨会话列内容钉还要解决预览代价（读别的会话的消息才能显示预览），为这个边缘需求加一个会话读取通道不值。
