...
现有面板骨架不动：标题行、提示行、色板行。列表的原子是**会话**不是钉：行钉与内容钉的 key 集取并集，只有消息钉的会话也进列表。初版只按行钉列会话、且把内容钉单列一段"本会话消息"——结果是只钉了消息的会话在列表里整个不出现，会话条目上也看不到消息钉的颜色；经用户校准改为聚合形态：每个条目显示会话名/预览，加两种色点并排（行钉点 14px、消息钉点 12px，hover 显 ×、点击移除），色点之后不再挂消息级预览。

不单列内容段是刻意的：面板只回答"哪个会话值得回"，消息级回看交给会话流里的图钉视觉本身。按消息逐条列内容钉是把会话流的信息在面板里复制一遍，用户校准为冗余。色板 filter tab 对两种色点同时生效：按某色过滤后两种点皆空的条目不列。

列出口径与渲染口径一致：会话行须在当前 DOM 中（visiblePaths，与会话钉同）；内容钉的孤儿判定见 §3.3。

### 6.2 点击定位

条目点击 = 打开会话；十字按钮 = 滚到会话行并高亮（会话钉既有语义）。面板不提供消息级跳转——`timeline:scrollTo` 通道本身仍在（session-tree 的 locate 继续消费），图钉面板不消费它。

### 6.3 不跨项目

列表只列当前项目的会话（行须在侧栏 DOM 中）。两种钉的 key 都是 sessionPath，切项目自然过滤；孤儿数据不清，切回自动恢复。
