...
- **评论框内联插在被评论消息的正下方**，与时间线同流；唤起时 timeline 自动滚动到锚定消息（框永远可见）。形态沿用 rewind 内联框（`data-rewind-inline`）的先例：上一行是引用区（灰字斜体，显示完整引文快照），下一行输入区。**键位语义（2026-08-06 修订，用户定夺）**：**Enter = 入篮并立即发送**（经 `timeline:sendRequested` 走受管发送口，composer 正文+全部评论一次发出）；**失焦 = 仅入篮**（多条累积走这条，发送仍由 composer 发送键完成）；**Esc = 取消**。没有按钮——键位与 rewind 内联框的肌肉记忆不重学，只有 Enter 的语义从"入篮"升级为"入篮并发送"。草稿不需要仪式性的存废决定。
