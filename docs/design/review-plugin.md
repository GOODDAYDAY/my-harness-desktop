...
- **echoAttachments**（挂在乐观消息上的附件徽章数据）：与 `items` 同构的只读预览（seq、quote 预览、comment）。timeline 在 user 气泡下方渲染成徽章条，不做文本拼装；展示文案零 i18n（编号与引文自描述）。展示基于文件：sendMessage 瞬间按 `hash(实发全文)` 把徽章写进会话头行 `custom-pi-desktop.echoAttachments` 域（零事件依赖），重扫 JSONL / RPC 重放后按内容 hash 查回徽章、渲染层对比删除拼装片段还原正文，切会话/刷新/重启不丢（`echo-attachments-persist.md`）。
