...
- 与底座自行恢复的关系是幂等，不赌底座行为：灌入在执行层做差量——`setModel` / `setThinkingLevel` 经 ensureForSend 拿到实证快照（起进程即 sync）后比对进程实况，已持目标值即跳过 RPC（同值 set_model 会在时间线落 model_change 分隔线，是纯噪声）；底座若不恢复（开成默认模型），快照不符，灌入是唯一的纠正点。两种行为下结果一致，desktop 不需要探测底座属于哪种——差量只认实证快照，不假设恢复行为。
