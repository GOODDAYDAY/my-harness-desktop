...
- `sessions/`：sessions-list、session-tree、session-bookmarks、session-colors、timeline、message-blocks、markdown、mermaid、puml、graphviz、sub-agent、review、im-graph、retry
...
- **`codeBlockRenderers`**：围栏语言渲染槽。插件按语言认领文本块内部的围栏代码块——声明 `{id, languages, fileExtensions?, component, order?}` 静态走 manifest，消费方按 language（小写比较）解析分发（markdown 渲染器）或按扩展名查 fileExtensions（文件预览——映射知识归贡献方，与 fileIcons 槽同构，新增图语言不动文件预览），组件 props 契约 `{code, streaming?}`，解析失败/流式未闭合由组件内部自降级源码。内置批次由 mermaid、puml、graphviz 插件贡献，第三方新增图语言不动 markdown。与 blockRenderers 的分工：blockRenderers 管整块类型，本槽管文本块内部的围栏语言。
