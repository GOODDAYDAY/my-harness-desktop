...
- **echo 回显持久化**：发送类附件回显（echoAttachments：echo 正文 + 徽章）随 `entryAppended` 落会话头行 custom 域（域级浅合并，entryId→回显，≤15 条 ≤3KB），`openSession`/`onSnapshot` 基线替换后按 entryId 回贴——切会话、压缩 resync 不丢评论徽章，气泡也不裸露拼装文本。
