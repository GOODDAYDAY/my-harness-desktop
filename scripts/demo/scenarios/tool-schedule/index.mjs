...
// 种子:见 seed.json。工具组不种——tool-manager 插件两层配置都空时自写
// PRESET_GROUPS(纯复用插件自身预设),剧本开关其内置组「只写」(write/edit/bash,
// 组名跨语言不变,字面锚点)。
...
export default {
  ...
  steps: [
    ...
    // 只读:关掉「只写」组(精确开关,不误伤其他组) → 预热一轮(下次发送生效) → 再发写请求 → 被拦
    { do: "click", target: { groupToggle: "只写" }, hold: 1200 },
    ...
    // 恢复:重开「只写」组(与关闭对称) → 预热 → 再发 → 成功
    { do: "click", target: { groupToggle: "只写" }, hold: 1000 },
    { do: "type", target: { css: "[data-timeline-composer]" }, submit: true, hold: 600, text: { "$t": "warmup" } },
    { do: "waitAgent", soft: true, opts: { appearMs: 8000 }, hold: 700 },
    { do: "type", target: { css: "[data-timeline-composer]" }, submit: true, text: { "$t": "writeReq" } },
    { do: "waitAgent", soft: true, opts: { appearMs: 8000 }, hold: 1500 },
