...
// 见 packages/shared/src/domain/events/session-state.ts 的 sessionEntryToNeutral 消费契约)。
// writer 负责三类结构不变量:parentId 链、timestamp 单调、文件头五项齐全——
// 内容(文案/工具参数)全部来自数据,机制与内容分离。
//
// bucket/文件名规则与生产同源:cwdToBucketName 镜像 packages/shared/src/domain/sessions.ts
// (脚本不 import TS);文件名 <ISO>_<uuid>.jsonl 决定 llm-recorder 落盘对齐。
import { randomUUID } from "node:crypto";
import { mkdirSync, writeFileSync } from "node:fs";

/** bucket 名规则与 packages/shared/src/domain/sessions.ts 的 cwdToBucketName 同源。 */
