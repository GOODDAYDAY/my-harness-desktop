...
// 数据源 = ctx.sessions.goal(内核无关的 GoalApi,状态机在 main 进程 GoalDriver)。
...
export function GoalBar(): React.ReactNode {
  const { sessions } = usePluginContext();
  const [goal, setGoal] = useState<GoalState | null>(null);
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState("");

  const goalApi = sessions.goal;

  useEffect(() => {
    if (!goalApi) return;
    let alive = true;
    void goalApi.get().then((g) => { if (alive) setGoal(g); });
    const off = goalApi.onChange((g) => {
      setGoal(g);
      if (g === null) { setEditing(false); setDraft(""); }
    });
    return () => { alive = false; off(); };
  }, [goalApi]);

  // 无能力/无目标不显示。
  if (!goalApi || goal === null) return null;

  const commitEdit = (): void => {
    const trimmed = draft.trim();
    if (trimmed === "") { setEditing(false); return; }
    void goalApi!.edit(trimmed);
    setEditing(false);
  };
  ...
  return (
    ...
    >
      ...
      {goal.phase === "active" ? (
        <button type="button" title="停止" onClick={() => void goalApi!.pause()} className="text-[var(--color-accent-warning)] hover:opacity-70">
          <Pause className="size-3.5" />
        </button>
      ) : (
        <button type="button" title="恢复" onClick={() => void goalApi!.resume()} className="text-[var(--color-accent-success)] hover:opacity-70">
          <Play className="size-3.5" />
        </button>
      )}
      <button type="button" title="关闭目标" onClick={() => void goalApi!.clear()} className="text-[var(--color-muted)] hover:opacity-70">
