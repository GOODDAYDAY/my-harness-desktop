...
export function registerBus(gateway: Gateway, ctx: MainContext): void {
  ...
  gateway.register(IPC.bus.status, (_e, pluginId: string) => {
    assertBusPermission(pluginId);
    return sessionBus.pluginStatus(pluginId);
  });
  gateway.register(IPC.bus.send, (_e, pluginId: string, to: string, kind: string, payload: unknown, replyTo?: string) => {
    assertBusPermission(pluginId);
    return sessionBus.pluginSend(pluginId, to, kind, payload, replyTo);
  });
  gateway.register(IPC.bus.sessionCreate, (_e, pluginId: string, opts: Record<string, unknown>) => {
    assertBusPermission(pluginId);
    return sessionBus.pluginSessionCreate(pluginId, opts);
  });
  gateway.register(IPC.bus.sessionAbort, (_e, pluginId: string, session: string) => {
    assertBusPermission(pluginId);
    return sessionBus.opSessionAbort(`plugin:${pluginId}`, session);
  });
  gateway.register(IPC.bus.channelMember, (_e, pluginId: string, channel: string, action: "join" | "leave", member?: string) => {
    assertBusPermission(pluginId);
    return sessionBus.pluginChannelMember(pluginId, channel, action, member);
  });
  gateway.register(IPC.bus.tapStart, (_e, pluginId: string, opts: Record<string, unknown>) => {
    assertBusPermission(pluginId);
