...
export function registerKernel(gateway: Gateway, ctx: MainContext): void {
  ...
  gateway.register(IPC.kernel.status, () =>
  ...
  gateway.register(IPC.kernel.fitPiExtensionAvailable, () => fitPiExtensionAvailable());
  gateway.register(IPC.kernel.listVersions, async (_e, forceRefresh: boolean) =>
  ...
  gateway.register(IPC.kernel.install, async (_conn, version: string) => {
  ...
  gateway.register(IPC.dshKernel.status, () =>
  ...
  gateway.register(IPC.dshKernel.listVersions, async (_e, forceRefresh: boolean) =>
  ...
  gateway.register(IPC.dshModels.get, () => ctx.dshConfigSource.listProviders());
  gateway.register(IPC.dshModels.set, async (_e, provider: string, detail: Omit<DshProvider, "provider">) => {
    await ctx.dshConfigSource.setProvider(provider, detail);
    return ctx.dshConfigSource.listProviders();
  });
  gateway.register(IPC.dshModels.removeProvider, async (_e, provider: string) => {
    await ctx.dshConfigSource.removeProvider(provider);
    return ctx.dshConfigSource.listProviders();
  });
  gateway.register(IPC.dshModels.renameProvider, async (_e, oldId: string, newId: string) => {
    await ctx.dshConfigSource.renameProvider(oldId, newId);
    return ctx.dshConfigSource.listProviders();
  });
  gateway.register(IPC.dshModels.getDefault, () => ctx.dshConfigSource.getDefaultModel());
  gateway.register(IPC.dshModels.setDefault, async (_e, sel: { provider: string; model: string; reasoningEffort?: string }) => {
    await ctx.dshConfigSource.setDefaultModel(sel);
    return ctx.dshConfigSource.getDefaultModel();
  });
  gateway.register(IPC.dshModels.test, (_e, cwd: string, provider: string, modelId: string) =>
    ctx.sessionStore.test(cwd, provider, modelId, "dsh"));
  // ---- IPC:dsh 配置(整份 ~/.dsh/settings.yaml 读写)----
  gateway.register(IPC.dshSettings.get, () => ctx.dshConfigSource.getSettings());
  gateway.register(IPC.dshSettings.set, async (_e, obj: Record<string, unknown>) => {
  ...
  gateway.register(IPC.piSettings.get, () => piSettings.get());
  gateway.register(IPC.piSettings.set, async (_e, patch: Record<string, unknown>) => {
  ...
  gateway.register(IPC.piSettings.schema, () => piSettings.schema());

  // ---- IPC:pi 内核 models(models.json,pi-model-manager 插件用)----
  gateway.register(IPC.models.get, () => modelsConfig.get());
  gateway.register(IPC.models.set, async (_e, config: unknown) => {
