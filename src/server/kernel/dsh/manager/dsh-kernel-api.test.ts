...
function makeConfig(initial: { providers: DshProvider[]; default: DshDefaultModel | null }) {
  ...
  const modelsApi = createDshModelsApi(api, {} as never);
  return { modelsApi, api, read: () => providers, def: () => def };
}
...
describe("createDshModelsApi.readConfig/saveConfig", () => {
  it("readConfig 返回 providers + default", async () => {
    const { modelsApi } = makeConfig({
      providers: [
        { provider: "us-new", baseURL: "https://x", apiKey: "sk-1", models: [{ id: "m1" }] },
      ],
      default: { provider: "us-new", model: "m1" },
    });
    const cfg = await modelsApi.readConfig();
    expect(cfg.providers).toHaveLength(1);
    expect(cfg.providers[0]).toMatchObject({ id: "us-new", baseUrl: "https://x", apiKey: "sk-1" });
    expect(cfg.default).toEqual({ provider: "us-new", model: "m1" });
  });

  it("saveConfig 全量 reconcile:增/改/删 + 设默认", async () => {
    ...
    await modelsApi.saveConfig({
      providers: [
        { id: "us-new", baseUrl: "https://y", apiKey: "sk-2", models: [{ id: "m1", name: "m1" }, { id: "m3", name: "m3" }] },
    ...
    expect(ids).toEqual(["added", "us-new"].sort());
    expect(read().find((p) => p.provider === "us-new")?.baseURL).toBe("https://y");
    expect(read().find((p) => p.provider === "us-new")?.apiKey).toBe("sk-2");
