// renderer React 应用主体 —— 壳骨架:标题栏 + 布局引擎 + 设置覆盖层。
// 由 index.tsx 的登录门(§8.2)放行后动态 import——本模块及其静态依赖在模块级
// 读 window.kernel,必须晚于 bootstrap 执行,不能做 HTML 直接入口。
//
...
// 引导虽已由 index.tsx 先行 await import,这里再 import 保证本模块单独加载也成立(模块幂等)。
import "./bootstrap";
import React, { memo, useEffect, useRef, useState } from "react";
import * as Tooltip from "@radix-ui/react-tooltip";
import { ThemeProvider } from "./app/theme-context";
import { initI18n, subscribeLocaleChange, i18next } from "./app/i18n-init";
  ...
  Promise.race([Promise.all([hydrateP, layoutHydrateP, initI18n(), pluginsReadyP]), timeoutP])
    ...
    .finally(() => {
      try {
        ...
        // 第 22 项:偏好多端同步——任一端 prefs.set 后服务端广播 prefs:changed,
        // 本端直写 zustand 状态(不走 setter,不回写 prefs → 无回声循环)。
        // 白名单只收外观类字段;导航/布局状态(lastCwd/sidebarWidth 等)不同步——
        // 各端独立导航(第 8/9 项:操作独立、状态同步)。
        const SYNCED_PREF_KEYS = new Set([
          "currentThemeId", "timelineThemeId", "fontScale",
          "fontMonoChoice", "fontEnglishChoice", "fontChineseChoice",
          "sidebarStyle", "sidepanelStyle",
          "sidebarFontScale", "sidepanelFontScale", "timelineFontScale",
        ]);
        void window.kernel.prefs.onChanged?.(({ key, value }) => {
          if (key === "currentLocale") {
            const loc = String(value);
            useUiStore.setState({ currentLocale: loc });
            void i18next.changeLanguage(loc);
            document.documentElement.lang = loc;
            return;
          }
          if (SYNCED_PREF_KEYS.has(key)) useUiStore.setState({ [key]: value } as never);
        });
