#!/usr/bin/env node
// Patch Electron.app's Info.plist + icon so dev mode shows "π Desktop" instead of "Electron".
...
try {
  execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName π Desktop" "${appPlist}"`, { stdio: "pipe" });
  execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName π Desktop" "${appPlist}"`, { stdio: "pipe" });
  ...
  console.log("[patch:electron] Patched Electron.app -> π Desktop (LaunchServices cache refreshed)");
} catch (e) {
